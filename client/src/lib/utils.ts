...
const BASE_URL = "https://api-laundry-web.edpos.vn"; // 👈 đổi theo domain backend của bạn
...
export async function completeOrderPayment(orderId, paymentData, apiRequest) {
  ...
  const response = await apiRequest("PUT", `https://api-laundry-web.edpos.vn/api/orders/${orderId}`, {
    status: "paid",
