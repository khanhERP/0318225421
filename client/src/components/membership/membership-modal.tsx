...
export function MembershipModal({ isOpen, onClose }: MembershipModalProps) {
  ...
  const { data: fetchedThresholds } = useQuery<{ GOLD: number; VIP: number }>({
    queryKey: ['https://api-demo.edpos.vn/api/membership-thresholds'],
    enabled: isOpen,
  });
  ...
  const updateThresholdsMutation = useMutation({
    mutationFn: async (newThresholds: { GOLD: number; VIP: number }) => {
      const response = await fetch('https://api-demo.edpos.vn/api/membership-thresholds', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['https://api-demo.edpos.vn/api/membership-thresholds'] });
      queryClient.invalidateQueries({ queryKey: ['https://api-demo.edpos.vn/api/customers'] });
      toast({
        title: "common.success",
