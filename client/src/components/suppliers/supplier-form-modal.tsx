...
export function SupplierFormModal({ isOpen, onClose, supplier }: SupplierFormModalProps) {
  ...
  const { data: suppliers } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', 'https://laundry-be-234a.onrender.com/api/suppliers');
      return response.json();
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertSupplier) => {
      ...
      const response = await apiRequest('POST', 'https://laundry-be-234a.onrender.com/api/suppliers', dataWithStoreCode);
      return response.json();
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: Partial<InsertSupplier>) => {
      const response = await apiRequest('PUT', `https://laundry-be-234a.onrender.com/api/suppliers/${supplier!.id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['https://laundry-be-234a.onrender.com/api/suppliers'] });
