...
export function EInvoiceModal({
  ...
  const completePaymentMutation = useMutation({
    mutationFn: ({
      ...
      return apiRequest("PUT", `api-demo.edpos.vn/api/orders/${orderId}/status`, {
        status: "paid",
  ...
  const { data: invoiceTemplates = [] } = useQuery<any[]>({
    queryKey: ["api-demo.edpos.vn/api/invoice-templates/active"],
    queryFn: async () => {
      try {
        const response = await apiRequest("GET", "api-demo.edpos.vn/api/invoice-templates/active");
        if (!response.ok) {
  ...
  const { data: orderData } = useQuery({
    queryKey: ["api-demo.edpos.vn/api/orders", orderId],
    queryFn: async () => {
      if (!orderId) return null;
      try {
        const response = await apiRequest("GET", `api-demo.edpos.vn/api/orders/${orderId}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  const handleGetTaxInfo = async () => {
    ...
    try {
      ...
      const response = await fetch("api-demo.edpos.vn/api/tax-code-lookup", {
        method: "POST",
        headers: {
  ...
  const handlePublishLater = async (event?: React.MouseEvent) => {
    ...
    try {
      ...
      // Lưu hóa đơn vào bảng invoices và invoice_items
      const invoiceResponse = await fetch("api-demo.edpos.vn/api/invoices", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (orderId) {
        try {
          ...
          const updateResponse = await fetch(`api-demo.edpos.vn/api/orders/${orderId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  const handleConfirm = async (event?: React.MouseEvent) => {
    ...
    // Call the proxy API
    const response = await fetch("api-demo.edpos.vn/api/einvoice/publish", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (result.success) {
      ...
      if (orderId) {
        ...
        try {
          ...
          const saveResponse = await fetch("api-demo.edpos.vn/api/orders", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
      ...
      try {
        const transactionResponse = await fetch("api-demo.edpos.vn/api/transactions", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
