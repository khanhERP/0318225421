...
export function ShoppingCart({
  ...
  const { data: userInfo } = useQuery({
    queryKey: ["user-info"],
    queryFn: async () => {
      const response = await fetch("https://laundry-be-234a.onrender.com/api/auth/me");
      if (!response.ok) {
  ...
  const fetchCustomers = async (searchTerm: string) => {
    try {
      ...
      const response = await fetch(
        `https://laundry-be-234a.onrender.com/api/customers?search=${encodeURIComponent(searchTerm)}`,
        {
          method: "GET",
          headers: {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderNumberResponse = await fetch("https://laundry-be-234a.onrender.com/api/orders/next-order-number");
    ...
    try {
      ...
      // Refresh orders list
      await queryClient.invalidateQueries({ queryKey: ["https://laundry-be-234a.onrender.com/api/orders"] });
