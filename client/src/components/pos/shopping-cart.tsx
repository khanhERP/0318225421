...
export function ShoppingCart({
  ...
  const { data: userInfo } = useQuery({
    queryKey: ["user-info"],
    queryFn: async () => {
      const response = await fetch("https://api-laundry-web.edpos.vn/api/auth/me");
      if (!response.ok) {
  ...
  const fetchCustomers = async (searchTerm: string) => {
    try {
      ...
      const response = await fetch(
        `https://api-laundry-web.edpos.vn/api/customers?search=${encodeURIComponent(searchTerm)}&status=all`,
        {
          method: "GET",
          headers: {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderNumberResponse = await fetch("https://api-laundry-web.edpos.vn/api/orders/next-order-number");
