...
export function ShoppingCart({
  ...
  const { data: userInfo } = useQuery({
    queryKey: ["user-info"],
    queryFn: async () => {
      const response = await fetch("api-demo.edpos.vn/api/auth/me");
      if (!response.ok) {
  ...
  const fetchCustomers = async (searchTerm: string) => {
    try {
      ...
      const response = await fetch(
        `api-demo.edpos.vn/api/customers?search=${encodeURIComponent(searchTerm)}&status=all`,
        {
          method: "GET",
          headers: {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderNumberResponse = await fetch("api-demo.edpos.vn/api/orders/next-order-number");
    ...
    try {
      ...
      const response = await fetch("api-demo.edpos.vn/api/orders", {
        method: "POST",
      ...
      // Refresh orders list
      await queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/orders"] });
