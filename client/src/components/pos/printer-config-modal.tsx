...
export function PrinterConfigModal({ isOpen, onClose }: PrinterConfigModalProps) {
  ...
  const createConfigMutation = useMutation({
    mutationFn: async (configData: any) => {
      const response = await apiRequest("POST", "https://bad07204-3e0d-445f-a72e-497c63c9083a-00-3i4fcyhnilzoc.pike.replit.dev/api/printer-configs", configData);
      return response.json();
    },
    onSuccess: () => {
      // Force refetch data
  ...
  const updateConfigMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const response = await apiRequest("PUT", `https://bad07204-3e0d-445f-a72e-497c63c9083a-00-3i4fcyhnilzoc.pike.replit.dev/api/printer-configs/${id}`, data);
      return response.json();
    },
    onSuccess: () => {
      // Force refetch data
  ...
  const testConnectionMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest("POST", `https://bad07204-3e0d-445f-a72e-497c63c9083a-00-3i4fcyhnilzoc.pike.replit.dev/api/printer-configs/${id}/test`);
      return response.json();
    },
    onSuccess: (data) => {
      toast({
