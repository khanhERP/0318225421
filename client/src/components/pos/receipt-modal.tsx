...
export function ReceiptModal({
  ...
  const { data: tableInfo } = useQuery({
    queryKey: ["api-demo.edpos.vn/api/tables/by-order", receipt?.id],
    queryFn: async () => {
      ...
      const orderResponse = await apiRequest(
        "GET",
        `api-demo.edpos.vn/api/orders/${receipt.id}`,
      );
      ...
      const tableResponse = await apiRequest(
        "GET",
        `api-demo.edpos.vn/api/tables/${order.tableId}`,
      );
  ...
  useEffect(() => {
    async function fetchPrinterConfigs() {
      try {
        const printerResponse = await fetch("api-demo.edpos.vn/api/printer-configs");
        ...
        if (receipt?.tableId) {
          try {
            const tableResponse = await fetch(`api-demo.edpos.vn/api/tables/${receipt.tableId}`);
            if (tableResponse.ok) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (activePrinterConfigs.length > 0) {
        ...
        try {
          const printResponse = await fetch("api-demo.edpos.vn/api/pos/print-receipt", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
