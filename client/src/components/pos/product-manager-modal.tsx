...
export function ProductManagerModal({
  ...
  const { data: units = [] } = useQuery<string[]>({
    queryKey: ["https://laundry-be-234a.onrender.com/api/products/units"],
    enabled: isOpen,
  });
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: (newProduct) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://laundry-be-234a.onrender.com/api/products/active"] });
  ...
  const updateProductMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`https://laundry-be-234a.onrender.com/api/products/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(finalData),
      });
