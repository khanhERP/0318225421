...
export function TableGrid({ onTableSelect, selectedTableId }: TableGridProps) {
  ...
  const { data: orders, refetch: refetchOrders } = useQuery({
    queryKey: ["api-demo.edpos.vn/api/orders"],
  ...
  const {
    ...
    queryKey: ["api-demo.edpos.vn/api/order-items", selectedOrder?.id || "none"],
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest("GET", `api-demo.edpos.vn/api/order-items/${orderId}`);
    ...
    const handleOrderUpdate = (event: CustomEvent) => {
      ...
      if (!event.detail?.skipAllRefetch && event.detail?.orderId) {
        queryClient.invalidateQueries({
          queryKey: ["api-demo.edpos.vn/api/order-items", event.detail.orderId],
        });
      }
    };
  ...
  const updateTableStatusMutation = useMutation({
    mutationFn: ({ tableId, status }: { tableId: number; status: string }) =>
      apiRequest("PUT", `api-demo.edpos.vn/api/tables/${tableId}/status`, { status }),
    onSuccess: async (data, variables) => {
      ...
      queryClient.removeQueries({ queryKey: ["api-demo.edpos.vn/api/tables"] });
  ...
  const completePaymentMutation = useMutation({
    mutationFn: ({
      ...
      apiRequest("PUT", `api-demo.edpos.vn/api/orders/${orderId}/status`, {
        status: "paid",
        paymentMethod,
      }),
    onSuccess: async (data, variables) => {
        ...
        try {
          ...
          if (otherActiveOrders.length === 0) {
            ...
            try {
              await apiRequest(
                "PUT",
                `api-demo.edpos.vn/api/tables/${completedOrder.tableId}/status`,
                {
                  status: "available",
                },
              );
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch("api-demo.edpos.vn/api/tables", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
          fetch("api-demo.edpos.vn/api/orders", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
        ]);
        ...
        queryClient.setQueryData(["api-demo.edpos.vn/api/tables"], freshTables);
        queryClient.setQueryData(["api-demo.edpos.vn/api/orders"], freshOrders);
      ...
      try {
        const [completedOrder, orderItemsData] = await Promise.all([
          queryClient.fetchQuery({
            queryKey: ["api-demo.edpos.vn/api/orders", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `api-demo.edpos.vn/api/orders/${variables.orderId}`,
              );
              return response.json();
            },
          }),
          queryClient.fetchQuery({
            queryKey: ["api-demo.edpos.vn/api/order-items", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `api-demo.edpos.vn/api/order-items/${variables.orderId}`,
              );
              return response.json();
            },
          }),
  ...
  const pointsPaymentMutation = useMutation({
    mutationFn: async ({
      ...
      // First redeem points
      await apiRequest("POST", "api-demo.edpos.vn/api/customers/redeem-points", {
        customerId,
        points,
      });

      // Then mark order as paid
      await apiRequest("PUT", `api-demo.edpos.vn/api/orders/${orderId}/status`, {
        status: "paid",
    ...
    onSuccess: async (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/orders"] });
      queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/tables"] });
      queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/customers"] });
      queryClient.invalidateQueries({
        queryKey: ["api-demo.edpos.vn/api/order-items", variables.orderId],
      });
      setOrderDetailsOpen(false);
  ...
  const deleteOrderMutation = useMutation({
    mutationFn: async (orderId: number) => {
      ...
      if (order?.tableId) {
        ...
        if (!otherActiveOrders || otherActiveOrders.length === 0) {
          await apiRequest("PUT", `api-demo.edpos.vn/api/tables/${order.tableId}/status`, {
            status: "available",
          });
        }
      }
    ...
    onSuccess: (data, orderId) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/tables"] });
      queryClient.invalidateQueries({
        queryKey: ["api-demo.edpos.vn/api/order-items", orderId],
  ...
  const recalculateOrderTotalMutation = useMutation({
    mutationFn: async (orderId: number) => {
      ...
      if (!remainingItems || remainingItems.length === 0) {
        ...
        const updateResult = await apiRequest("PUT", `api-demo.edpos.vn/api/orders/${orderId}`, {
          subtotal: "0",
          tax: "0",
          total: "0",
        });
    ...
    onSuccess: (data, orderId) => {
      ...
      Promise.all([
        fetch("api-demo.edpos.vn/api/orders", { cache: "no-store" }).then((r) => r.json()),
        fetch("api-demo.edpos.vn/api/tables", { cache: "no-store" }).then((r) => r.json()),
        fetch(`api-demo.edpos.vn/api/order-items/${orderId}`, { cache: "no-store" }).then((r) =>
          r.json(),
        ),
      ])
        .then(() => {
          ...
          queryClient.setQueryData(["api-demo.edpos.vn/api/orders"], (oldData: any) => {
  ...
  const handlePaymentMethodSelect = async (
    ...
    if (method === "paymentCompleted" && paymentData?.success) {
      ...
      try {
        ...
        setTimeout(() => {
          queryClient.refetchQueries({ queryKey: ["api-demo.edpos.vn/api/tables"] });
    ...
    try {
      ...
      if (currentOrderId) {
        try {
          const response = await apiRequest(
            "GET",
            `api-demo.edpos.vn/api/order-items/${currentOrderId}`,
          );
  ...
  const handlePrintOrder = async (order: any) => {
    ...
    try {
      const orderItems = await queryClient.fetchQuery({
        queryKey: [`api-demo.edpos.vn/api/order-items/${order.id}`],
        queryFn: async () => {
          const response = await apiRequest(
            "GET",
            `api-demo.edpos.vn/api/order-items/${order.id}`,
      ...
      const response = await fetch("api-demo.edpos.vn/api/auto-print", {
        method: "POST",
        headers: {
  ...
  const splitOrderMutation = useMutation({
    mutationFn: async (splitData: {
      ...
      const response = await apiRequest("POST", "api-demo.edpos.vn/api/orders/split", {
      ...
      await Promise.all([
        ...
        refetchTables(),
        queryClient.invalidateQueries({
          queryKey: ["api-demo.edpos.vn/api/order-items", splitData.originalOrderId],
        }),
      ]);
  ...
  const handleSplitOrder = async (splitData: any) => {
    try {
      ...
      const response = await apiRequest("POST", "api-demo.edpos.vn/api/orders/split", splitData);
      ...
      <OrderDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open && editingOrder) {
            ...
            queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/tables"] });
            queryClient.invalidateQueries({
              queryKey: ["api-demo.edpos.vn/api/order-items", editingOrder.id],
            });
