...
export function DashboardOverview() {
  ...
  const {
    ...
    queryKey: ["https://laundry-be-234a.onrender.com/api/orders/date-range", startDate, endDate, "all"],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://laundry-be-234a.onrender.com/api/orders/date-range/${startDate}/${endDate}/all`,
        );
        if (!response.ok) {
  ...
  const handleRefresh = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["https://laundry-be-234a.onrender.com/api/orders/date-range"] });
  ...
  const { data: allCurrentOrders = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://laundry-be-234a.onrender.com/api/orders");
        if (!response.ok)
          throw new Error(`HTTP error! status: ${response.status}`);
        return await response.json();
      } catch (error) {
