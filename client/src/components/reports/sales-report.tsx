...
export function SalesReport() {
  ...
  const {
    ...
    queryKey: ["https://api-laundry-web.edpos.vn/api/orders/date-range", startDate, endDate, "all"],
    queryFn: async () => {
      try {
  ...
  const {
    ...
    queryKey: ["https://api-laundry-web.edpos.vn/api/order-items/date-range", startDate, endDate, "all"],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://api-laundry-web.edpos.vn/api/order-items/${startDate}/${endDate}`,
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
