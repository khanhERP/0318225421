...
export function OrderReport() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`https://bad07204-3e0d-445f-a72e-497c63c9083a-00-3i4fcyhnilzoc.pike.replit.dev/api/orders/date-range/${startDate}/${endDate}`);
        if (!response.ok) {
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`https://bad07204-3e0d-445f-a72e-497c63c9083a-00-3i4fcyhnilzoc.pike.replit.dev/api/transactions/${startDate}/${endDate}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
