...
export function OrderReport() {
  ...
  const { data: transactions = [] } = useQuery({
    queryKey: ["https://870b3a74-08b9-4ccf-b28f-dc7e4de678a7-00-2rac59553o6xa.sisko.replit.devapi/transactions", startDate, endDate],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://870b3a74-08b9-4ccf-b28f-dc7e4de678a7-00-2rac59553o6xa.sisko.replit.devapi/transactions/${startDate}/${endDate}`,
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
