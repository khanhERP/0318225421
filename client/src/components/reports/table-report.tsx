...
export function TableReport() {
  ...
  const { data: tables = [], isLoading: tablesLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://api-demo.edpos.vn/api/tables");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: transactionItems = [], isLoading: transactionItemsLoading } =
    useQuery({
      queryKey: ["https://api-demo.edpos.vn/api/transaction-items"],
      queryFn: async () => {
        try {
          const response = await fetch("https://api-demo.edpos.vn/api/transaction-items");
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          const data = await response.json();
