...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone }: CustomerFormModalProps) {
  ...
  const { data: orderHistory, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `api-demo.edpos.vn/api/orders`);
  ...
  const generateCustomerId = async () => {
    try {
      const response = await apiRequest("GET", "api-demo.edpos.vn/api/customers/next-id");
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      ...
      const response = await apiRequest("POST", "api-demo.edpos.vn/api/customers", cleanData);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      ...
      const response = await apiRequest("PUT", `api-demo.edpos.vn/api/customers/${customer.id}`, cleanData);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["api-demo.edpos.vn/api/customers"] });
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    if (customer?.id) {
      ...
      if (!customerIdToUse || customerIdToUse === t('common.autoGenerated')) {
        try {
          const response = await fetch('api-demo.edpos.vn/api/customers/next-id');
